...
const byte socAlto = 79;         //77 Valor a mantener Maximo de SOC
const byte limiteCarga = 80;     //80 Comienza a descargar
const byte voltajeMinimo = 229;  // 230
const byte voltajeMaximo = 239;  // 240
const byte marcaBaja = 70;       // Este valor tambien condiciona a partir de carga voltajeMaximo
...
const unsigned int tiempoActivoA0 = 5000;     // 0.5 segundos en milisegundos
const unsigned int tiempoInactivoA0 = 15000;  // 30 segundos en milisegundos
...
void mensajeSOC() {
  ...
  // uint8_t data6[] = { 0x69, 0x7D, 0x00, 0x92, 0x21, 0x20, 0x8F };
  uint8_t data6[] = { 0x67, 0x64, 0x00, 0x99, 0x15, 0x13, 0x61 };
  for (int i = 0; i < 10; i++) {  // Número repeticiones enviado el mensaje.
    // Envía el mensaje
    sendCANMessage(0x3CB, 7, data6);
    sentMessageCount++;
  }
}
// 8 ms   03B      5 00 05 00 ED 32 Itensidad y voltaje
// 100 ms 3C9      8 03 FF 25 02 9A 03 22 BC 0x21 ( chg with power )
// 100 ms 3CB      7 67 64 00 99 15 13 61    SOC
// 100 ms 3CD      5 00 00 00 EC C1          Voltaje del paquete: 16 bits, sin signo [V] SOC
...
void processCanMessage(CAN_FRAME &incoming) {
  ...
  if (incoming.id == 0x3CB && incoming.length == 7) {
    ...
    if (incoming.id == 0x3CB && (hvSocValue < marcaBaja || hvSocValue > marcaAlta)) {
      // Llamar a la función mensajeSOC
      mensajeSOC();
    }
...
void leerCan() {
  ...
  if (Can0.available()) {
    ...
    if ((incoming.id == 0x3CB && incoming.length == 7) || (incoming.id == 0x03B && incoming.length == 5) || (incoming.id == 0x529 && incoming.length == 7) || (incoming.id == 0x3CA && incoming.length == 5) || (incoming.id == 0x244 && incoming.length == 8) || (incoming.id == 0x3C8 && incoming.length == 5)) {
