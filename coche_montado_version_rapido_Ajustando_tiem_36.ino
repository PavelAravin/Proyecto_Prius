...
const byte socAlto = 77;         //77 Valor a mantener Maximo de SOC
const byte limiteCarga = 80;     //80 Comienza a descargar
const byte voltajeMinimo = 220;  // 215
const byte voltajeMaximo = 230;  // 230
const byte marcaBaja = 60;       // Este valor tambien condiciona a partir de carga voltajeMaximo
...
const unsigned int tiempoActivoA0 = 250;      // 0.5 segundos en milisegundos
...
void loop() {
  ...
  if (switchState == HIGH) {
    avisoRc();
    // Verifica que al menos uno de los pines D3 o D4 esté bajo y cambia su estado a alto
    if (digitalRead(outputPinD3) == LOW || digitalRead(outputPinD4) == LOW) {
      // Cambia los pines D3 y D4 a alto
      digitalWrite(outputPinD3, HIGH);
      digitalWrite(outputPinD4, HIGH);
    }
  }
