...
MCP2515 VCC a 5V del Arduino Due.
MCP2515 GND a GND del Arduino Due.
MCP2515 SCK un pin SPI SCK (pin 76).
MCP2515 SO un pin SPI MISO (pin 74).
MCP2515 SI un pin SPI MOSI (pin 75).
MCP2515 CS a un pin de tu elección (por ejemplo, pin 10) 
y asegurate de que coincida con la configuración en tu código.
MCP2515 INT a un pin de tu elección que soporte interrupciones 
(por ejemplo, pin 2) y asegurate de que coincida con la configuración en tu código.
*/
...
const int SPI_CS_PIN = 10;  // Define el pin CS del MCP2515
const int INT_PIN = 2;
...
void setup() {
  ...
  //   CAN.begin(MCP_STDEXT, CAN_500KBPS); // Inicializa la biblioteca y configura la velocidad CAN.

  if (CAN.begin(MCP_STDEXT, CAN_125KBPS, MCP_16MHZ) == CAN_OK) {
...
void solicitudDatOBD() {
  byte canData[8] = { 0x02, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
  unsigned long canId = 0x7DF;

  CAN.sendMsgBuf(canId, 0, 8, canData);  // Envía el mensaje CAN
  sentMessageCount++;
}
/*
...
*/
void inicioObd1() {
  byte canData[8] = { 0x02, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
  unsigned long canId = 0x7DF;
...
void inicioObd() {
  byte canData[8] = { 0x02, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
  unsigned long canId = 0x18DB33F1;
...
*/
void errorMotor() {
  byte canData[8] = { 0x02, 0x3E, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
  unsigned long canId = 0x7E0;
...
*/
void erroresBat1() {
  byte canData[8] = { 0x01, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
  unsigned long canId = 0x7E2;
...
*/
void erroresBat2() {
  byte canData[8] = { 0x01, 0x44, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
  unsigned long canId = 0x7EA;
...
*/
void erroresBat4() {
  byte canData[8] = { 0x01, 0x044, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
  unsigned long canId = 0x7EB;
...
*/
void solicitudDat1() {
  byte canData[8] = { 0x24, 0x13, 0x13, 0x13, 0x13, 0x00, 0x00, 0x00 };
  unsigned long canId = 0x7EB;
...
*/
void activateEVMode() {
  byte canData[7] = { 0xA8, 0x00, 0x00, 0x85, 0x40, 0x00, 0x00 };
  unsigned long canId = 0x529;

  CAN.sendMsgBuf(canId, 0, 7, canData);  // Envía el mensaje CAN
  sentMessageCount++;
}
/*
void activateEVMode() {
...
void deactivateEVMode() {
  byte canData[7] = { 0xA8, 0x00, 0x00, 0x85, 0x00, 0x00, 0x00 };
  unsigned long canId = 0x529;
...
void processCanMessage() {
  ...
  if (CAN.checkReceive() == CAN_MSGAVAIL) {
    INT32U canId;
    INT8U canLen;
    INT8U canData[MAX_CHAR_IN_MESSAGE];
...
void activaEv() {
  ...
  unsigned long tiempoSinDatos = currentMillisEv - tiempoUltimaAplicacion;

  // Verificar si hay una activación pendiente basada en EV y el tiempo de espera
  if (EV == "NO" && (activacionPendiente || tiempoSinDatos >= tiempoEsperar)) {
  ...
  if (activacionPendiente && currentMillisEv - ultimaActivacion >= evTiactivado) {
    ...
    digitalWrite(outputPinA1, LOW);
    activacionPendiente = false;
  }

  // Actualiza el tiempo de última aplicación
  tiempoUltimaAplicacion = currentMillisEv;
}
